...
class SRG_Bottleneck(nn.Module):
  ...
  def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3)):
    ...
    if c2 != c1:
      c2 = c1
    c_ = c2//2
    self.repghostconv1 = RepGhostModule(c_, c_)
    self.repghostconv2 = RepGhostModule(c2, c2)
    self.add = shortcut and c1 == c2
...
class CSRG(C3):
    def __init__(self, c1, c2, n=3, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(SRG_Bottleneck(c_, c_) for _ in range(n)))
